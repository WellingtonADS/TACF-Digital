...
CREATE OR REPLACE FUNCTION public.confirmar_agendamento(p_user_id uuid, p_session_id uuid)
 RETURNS TABLE(success boolean, booking_id uuid, error text, order_number text)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION public.create_location(p_name text, p_address text, p_max_capacity integer, p_status text DEFAULT 'active'::text, p_facilities text[] DEFAULT ARRAY[]::text[], p_metadata jsonb DEFAULT NULL::jsonb)
 RETURNS locations
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
    result public.locations%ROWTYPE;
BEGIN
    INSERT INTO public.locations (name, address, max_capacity, status, facilities, metadata, created_by)
    VALUES (p_name, p_address, p_max_capacity, p_status, p_facilities, p_metadata, auth.uid())
...
CREATE OR REPLACE FUNCTION public.fill_sessions_date_from_starts_at()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
  IF NEW.date IS NULL AND NEW.starts_at IS NOT NULL THEN
    NEW.date := (NEW.starts_at AT TIME ZONE 'UTC')::date;
  END IF;
  IF NEW.period IS NULL AND NEW.starts_at IS NOT NULL THEN
    IF EXTRACT(HOUR FROM NEW.starts_at AT TIME ZONE 'UTC') < 12 THEN
      NEW.period := 'morning'::session_period;
    ELSE
      NEW.period := 'afternoon'::session_period;
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE OR REPLACE FUNCTION public.fn_sync_session_time_metadata()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_audit_logs()
 RETURNS TABLE(id uuid, action text, entity text, user_id uuid, user_name text, created_at timestamp with time zone, details text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_locations(p_search_term text DEFAULT NULL::text, p_status text DEFAULT NULL::text, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, name text, address text, max_capacity integer, status text, facilities text[], metadata jsonb, created_by uuid, created_at timestamp with time zone, updated_at timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
 STABLE
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.get_results_history(p_limit integer, p_cursor text, p_from date DEFAULT NULL::date, p_to date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  IF p_cursor IS NOT NULL THEN
    ...
    FROM (
      SELECT b.id, b.user_id AS profile_id, p.full_name, p.saram, b.score, b.result_details, b.test_date, b.created_at
      FROM public.bookings b
      LEFT JOIN public.profiles p ON p.id = b.user_id
      WHERE (b.created_at, b.id) < ((v_arg->>'created_at')::timestamptz, (v_arg->>'id')::uuid)
  ...
  ELSE
    IF jsonb_array_length(v_rows) = p_limit THEN
      v_has_more := true;
      v_next_cursor := (
        SELECT encode(convert_to(jsonb_build_object('created_at', (elem->>'created_at')::text, 'id', elem->>'id')::text, 'utf8'), 'base64')
  ...
  RETURN jsonb_build_object('rows', v_rows, 'next_cursor', v_next_cursor, 'has_more', v_has_more);
END;
$function$


CREATE OR REPLACE FUNCTION public.get_sessions_availability(p_start date, p_end date)
 RETURNS TABLE(session_id uuid, date date, period text, max_capacity integer, occupied_count bigint, available_count bigint)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.locations_updated_at()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.update_location(p_id uuid, p_name text DEFAULT NULL::text, p_address text DEFAULT NULL::text, p_max_capacity integer DEFAULT NULL::integer, p_status text DEFAULT NULL::text, p_facilities text[] DEFAULT NULL::text[], p_metadata jsonb DEFAULT NULL::jsonb)
 RETURNS locations
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
CREATE OR REPLACE FUNCTION public.update_session_scores(p_session_id uuid, p_user_id uuid, p_attendance_confirmed boolean)
 RETURNS TABLE(success boolean, error text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
