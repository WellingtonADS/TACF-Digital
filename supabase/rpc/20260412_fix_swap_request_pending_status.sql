-- Corrige a criacao de solicitacoes de reagendamento para sempre persistir
-- o status pendente explicito e normaliza registros legados nulos.

update public.swap_requests
set status = 'solicitado'::public.swap_status
...
begin
  ...
  if exists (
    ...
    where sr.booking_id = p_booking_id
      and coalesce(sr.status, 'solicitado'::public.swap_status) =
        'solicitado'::public.swap_status
  ) then
