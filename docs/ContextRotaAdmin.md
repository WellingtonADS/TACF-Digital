...
- **→ Gestão de Reagendamento** (`/app/reagendamentos`): Análise de pedidos de troca de data (Deferir/Indeferir).
- **→ Justificativa Selecionada:** Visualização de documentos anexos pelo militar.

- **Reagendamentos (submenu sob Agendamentos):** Link rápido disponibilizado em toda a aplicação para que o usuário acesse a funcionalidade de solicitar troca. Na interface militar, o botão "Solicitar Reagendamento" também aparece diretamente no cartão "Próximo Evento" e na tabela de histórico.
...
- **→ Perfis de Acesso:** Definição de permissões (Visualizar, Criar, Editar, Excluir) por módulo. Rota `/app/configuracoes/perfis`.
...
- **Perfis de Acesso (Gestão de Perfis):** Atribuição de permissões (Visualizar, Criar, Editar, Excluir) para perfis como Administrador Central, Oficial de Treinamento e Junta de Saúde. rota `/app/configuracoes/perfis`
...
- **→ Reagendamento de Eventos:** Painel lateral (drawer) que permite ao militar informar nova data, justificativa e anexar comprovante; as solicitações são gravadas em `swap_requests` e aparecem com marcador "Pendente".
...
- **Gestão de Reagendamento** (`/app/reagendamentos`): Área para administradores analisarem pedidos de nova data para testes (Deferir/Indeferir).
