...
export default function OmLocationEditor({
  ...
  const existing = useMemo(() => {
    if (isNew || !resolvedId) return null;
    return locations.find((l) => l.id === resolvedId) || null;
  }, [isNew, resolvedId, locations]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (isNew) {
        ...
        const loc = await update(resolvedId, data);
        if (loc) {
  ...
  const content = (
    <div className={`${embedded ? "" : "mx-auto max-w-5xl"} w-full pb-16`}>
      <PageHero
        isNew={isNew}
        title={
          isNew
